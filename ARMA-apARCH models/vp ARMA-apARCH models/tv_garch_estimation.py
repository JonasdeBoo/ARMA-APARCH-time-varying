...
class QuasiMaximumLikelihoodEstimator:
    ...
    def _set_bounds(self):
        ...
        if self.model_type == 'mean-x':
            ...
            bnds += ((-2 * np.var(self.df[self.returns_col]), 2 * np.var(self.df[self.returns_col])),)  # bounds on omega_star
            bnds += ((e, 1 - e),)                                               # bounds on beta
            bnds += ((-0.999, 1-e),)                                            # bounds on beta_star
            bnds += ((e, 10),)                                                  # bounds on alpha
            bnds += ((-0.9999, 1-e), )                                          # bounds on alpha_star
            ...
            bnds += ((e, 2 * np.var(self.df[self.returns_col])),)               # bounds on omega
            bnds += (
            (-2 * np.var(self.df[self.returns_col]), 2 * np.var(self.df[self.returns_col])),)  # bounds on omega_star
            bnds += ((e, 1 - e),)                                               # bounds on beta
            bnds += ((-0.999, 1 - e),)                                          # bounds on beta_star
            bnds += ((e, 10),)                                                  # bounds on alpha
            bnds += ((-0.9999, 1 - e),)                                         # bounds on alpha_star
            bnds += ((-(1 - e), 1 - e),) * 2                                    # bonds on phi
            bnds += ((e, 10), (-0.9999, 10)) * (np.sum(self.lags_exog) + np.sum(self.x_garch_lags))        # exogenous bounds
            ...
            bnds += ((e, 10), (-0.9999, 10)) * (np.sum(self.lags_exog))         # bounds on pi_ik
